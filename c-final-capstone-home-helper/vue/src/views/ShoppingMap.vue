  ...
  created() {
    ...
    if (this.$route.params.id.includes("a")) {
      UserService.getAppliance(this.itemId).then((response) => {
        ...
        this.url = `https://www.google.com/maps/embed/v1/search?key=hdgxfcyvubhinjomioyu5y5ex_2s&q=appliance+store+${this.urlItem},${this.urlZip}`;
      });
    } else {
      UserService.getAppliance(this.itemId).then((response) => {
        this.items = response.data;
        ...
        this.urlItem = this.items[0].name;
        this.homeId = this.items[0].homeId;
        this.url = `https://www.google.com/maps/embed/v1/search?key=sdcvmb,inmyugfbtcvxt,89t5ffb5n6gm_2s&q=${this.urlItem}+servie,${this.urlZip}`;
      });
    }
